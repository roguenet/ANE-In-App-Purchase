		...
		private function onStatus(event:StatusEvent):void
		{
			...
			switch(event.code)
			{
				...
				case "RESTORE_INFO_RECEIVED":
					var jsonRestoreInfo:String = event.level;
					try
					{
						var restoreInfo:Object = JSON.parse(jsonRestoreInfo);
						for each (var purchase:Object in restoreInfo.purchases) 
						{
							var signedData:Object = JSON.parse(purchase.receipt.signedData);
							purchase.productId = signedData.productId;
						}
						jsonRestoreInfo = JSON.stringify(restoreInfo);
					} 
					catch(error:Error)
					{
						trace("[InAppPurchase] Couldn't parse purchase: " + jsonRestoreInfo); 
					}
					e = new InAppPurchaseEvent(InAppPurchaseEvent.RESTORE_INFO_RECEIVED, jsonRestoreInfo);
					break;
				default:
